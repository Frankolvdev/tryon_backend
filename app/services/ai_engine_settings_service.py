...
class AiEngineSettingsService:
    ...
    def get(self, db: Session) -> AiEngineSettingsResponse:
        v = self._integer_values(db)
        s = self._string_values(db)
        dispatchers = v["generation_runpod_dispatch_workers"]
        in_flight = v["generation_runpod_max_in_flight"]
        return AiEngineSettingsResponse(
            local_parallel_executions=v["generation_local_parallel_executions"],
            runpod_min_workers=v["runpod_min_workers"],
            runpod_max_workers=v["runpod_max_workers"],
            runpod_dispatch_workers=dispatchers,
            runpod_max_in_flight=in_flight,
            modal_gpu=s["modal_gpu"],
            modal_min_containers=v["modal_min_containers"],
            modal_max_containers=v["modal_max_containers"],
            modal_concurrency=v["modal_concurrency"],
            ...
            effective_runpod_parallelism=min(dispatchers, in_flight),
    ...
    def update(self, db: Session, data: AiEngineSettingsUpdate) -> AiEngineSettingsResponse:
        incoming = {
            ...
            "modal_min_containers": data.modal_min_containers,
            "modal_max_containers": data.modal_max_containers,
            "modal_concurrency": data.modal_concurrency,
            "modal_input_concurrency": data.modal_input_concurrency,
        ...
        existing = {row.key: row for row in db.scalars(select(SystemSetting).where(SystemSetting.key.in_(keys))).all()}

        for order, (key, value) in enumerate(incoming.items(), start=10):
            row = existing.get(key)
            if row is None:
                label, description = LABELS.get(key, (key, key))
                row = SystemSetting(
                    category="ai",
                    key=key,
                    label=label,
                    ...
                    value_type="integer",
                    default_value_integer=INTEGER_DEFAULTS[key],
                    is_public=False,
                    is_editable=True,
                    is_sensitive=False,
                    requires_restart=True,
                ...
                db.add(row)
            # This assignment must also run for existing rows.
            row.value_integer = value

        row = existing.get("modal_gpu")
        if row is None:
            label, description = LABELS["modal_gpu"]
            row = SystemSetting(
                category="ai",
                key="modal_gpu",
                label=label,
            ...
            db.add(row)
        # This assignment must also run for an existing row.
        row.value_string = data.modal_gpu
