...
class GenerationModuleRuntimeService:
    ...
    def create(
        ...
        *,
        ...
        # The pricing rule is the single source of truth for token quantity.
        # FIFO lots are used only after this amount is known, to allocate the
        # debit and preserve each lot's discount/profit snapshot.
    ...
    def _finalize_dynamic_billing(self, db: Session, item: GenerationModuleExecutionResponse, *, pending_settlement: bool = False) -> None:
        ...
        if item.user_id is not None and final_price is not None:
            try:
                ...
                # The token amount is already fixed by the pricing rule. A lack
                # of balance only delays the remaining debit; it must never make
                # discounted/subscription lots recalculate or inflate that amount.
                charged_tokens = int(item.tokens_charged or 0)
                pending_tokens = max(int(final_tokens) - charged_tokens, 1)
                ...
                current_ledger = token_value_ledger_service.execution_summary(
                    db, str(item.id), expected_tokens=charged_tokens
                )
                item.result_locked = True
                item.billing_access_status = "payment_pending"
                item.estimated_pending_tokens = pending_tokens
                item.billing_breakdown = {
                    **pending_snapshot,
                    "finalized": False,
                    "provider": provider,
                    ...
                    "desired_profit_per_token_usd": profit_per_token_usd,
                    "desired_profit_usd": round(float(applied_profit_usd or 0), 9),
                    "applied_profit_usd": round(float(current_ledger.get("company_profit_usd") or 0), 9),
                    ...
                    "estimated_final_tokens": int(final_tokens),
                    "estimated_pending_tokens": pending_tokens,
                    "pending_tokens_not_charged": pending_tokens,
                    "extra_tokens_debited": 0,
                    "tokens_refunded": 0,
                    "termination_status": item.status,
                    "pricing_rule_id": item.pricing_rule_id,
                    "settlement_pending": True,
                    "settlement_reason": "insufficient_token_balance",
                    "result_locked": True,
                    "billing_access_status": "payment_pending",
                    "token_charge_basis": "current_pricing_rule_then_fifo_allocation",
                    "token_bag_charge_quote": current_ledger.get("allocations"),
                    "token_bag_traceability_status": current_ledger.get("traceability_status"),
                }
                item.logs.append(GenerationModuleExecutionLog(
                    timestamp=utc_now(),
                    level="warning",
                    message=(
                        "Result locked because the final token charge exceeds "
                        "the user's available token balance."
                    ),
                ))
        ...
        item.billing_breakdown = {
            **estimated_snapshot,
            ...
            "final_tokens": final_tokens,
            "token_charge_basis": "current_pricing_rule_then_fifo_allocation",
            "token_bag_charge_quote": ledger_summary.get("allocations"),
            "token_bag_traceability_status": ledger_summary.get("traceability_status"),
            "infrastructure_capacity_from_bags_usd": None,
