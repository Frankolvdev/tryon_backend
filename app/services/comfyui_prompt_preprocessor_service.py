...
class ComfyUIPromptPreprocessorService:
    ...
    @classmethod
    def preprocess_with_result(cls, prompt: dict[str, Any]) -> PromptPreprocessorResult:
        ...
        return PromptPreprocessorResult(prompt=normalized, normalized_values=count)
    ...
    @classmethod
    def _walk(cls, value: Any, *, parent_key: str | None) -> tuple[Any, int]:
        ...
        if isinstance(value, list):
            ...
            output_list = []
            for item in value:
                normalized_item, count = cls._walk(item, parent_key=parent_key)
        ...
        if isinstance(value, str) and cls._should_normalize_value(parent_key, value):
    ...
    @classmethod
    def _should_normalize_dict_key(cls, *, parent_key: str | None, key: str) -> bool:
        if "\\" not in key:
            return False
    ...
    @classmethod
    def _should_normalize_value(cls, key: str | None, value: str) -> bool:
        if "\\" not in value:
            return False
        normalized_key = str(key or "").strip().lower()
        if normalized_key in cls._PATH_KEYS or cls._PATH_KEY_PATTERN.search(normalized_key):
            return True
    ...
    @classmethod
    def assert_no_windows_model_paths(cls, prompt: dict[str, Any]) -> None:
        invalid: list[str] = []
