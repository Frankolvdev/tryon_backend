...
class StripeClientService:
    ...
    def delete_customer_with_subscriptions(
        self,
        db: Session,
        *,
        customer_id: str,
    ) -> dict[str, Any]:
        """Cancel every Stripe subscription and delete the customer.

        A missing or already deleted customer is treated as successfully
        cleaned so an interrupted previous deletion can be retried safely.
        Any other Stripe failure is propagated and prevents local deletion.
        """
        self._get_config(db)
        ...
        if bool(getattr(customer, "deleted", False)):
            return {
                "customer_id": customer_id,
                "customer_deleted": True,
                "already_absent": True,
        ...
        if not bool(getattr(deleted_customer, "deleted", False)):
            raise ConflictException(
