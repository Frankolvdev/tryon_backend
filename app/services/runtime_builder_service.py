...
"""
    ...
    @staticmethod
    def _modal_snapshot_warmup_workflow(
        resident_models: list[str] | tuple[str, ...] | None = None,
    ) -> str:
        selected = [
            str(item).strip().replace("\\", "/")
        ...
        for index, model_path in enumerate(selected, start=1):
            category, _, filename = model_path.partition("/")
            category = category.casefold()
            loader_id = f"runtime-resident-{index}"
            if category in {"diffusion_models", "unet", "unets"}:
                workflow[loader_id] = {
            ...
            elif category in {"text_encoders", "clip"}:
                workflow[loader_id] = {
            ...
            workflow[f"{loader_id}-sink"] = {
                "class_type": "TryonSnapshotWarmupSink",
                "inputs": {"value": [loader_id, 0]},
            }
...
"""
        ...
        if modal_enabled:
            result["modal_runtime_engine_toml"] = (
                RuntimeBuilderService._modal_runtime_engine_toml(
                    volume_path, modal_resident_models
                )
            )
            result["modal_snapshot_warmup_workflow"] = (
                RuntimeBuilderService._modal_snapshot_warmup_workflow(
                    modal_resident_models
                )
            )
