...
from app.services.beam_file_manager_service import BeamFileManagerError as E, BeamFileManagerService as S

router=APIRouter(prefix="/beam-file-manager",dependencies=[Depends(admin_guard)])
...
async def upload(request:Request,volume:str=Query(...),path:str=Query(""),overwrite:bool=Query(True),db:Session=Depends(get_db)):
    ...
    fd,tmp_name=tempfile.mkstemp(prefix="tryon-beam-upload-",suffix="-"+Path(filename).name); os.close(fd)
    size=0
    try:
        ...
        with open(tmp_name,"rb") as source:
            return call(S.upload_file,db,volume,destination,Path(tmp_name))
