...
def get_user_security(
    ...
    statement = select(UserAccountSecurity).where(
        UserAccountSecurity.user_id == user_id
    )
...
def synchronize_superadmin(
    ...
    if user.deleted_at is not None:
        ...
        updated_fields.append("deleted_at")
...
def synchronize_superadmin_security(
    ...
    if security is None:
        security = UserAccountSecurity(
            ...
            locked_until=None,
            terms_accepted=True,
            terms_version="development-seed",
        ...
        print(
            "[SEED] Superadmin account security created."
        )
    ...
    if security.account_status != "active":
    ...
    if security.verification_required:
        ...
        updated_fields.append("verification_required")

    if security.failed_login_attempts != 0:
        security.failed_login_attempts = 0
        updated_fields.append("failed_login_attempts")
    ...
    if not security.terms_accepted:
        ...
        updated_fields.append("terms_accepted")

    if not security.terms_version:
        security.terms_version = "development-seed"
        updated_fields.append("terms_version")

    if security.terms_accepted_at is None:
    ...
    if not security.age_confirmed:
        ...
        updated_fields.append("age_confirmed")

    if security.age_confirmed_at is None:
        security.age_confirmed_at = now
        updated_fields.append("age_confirmed_at")
    ...
    if updated_fields:
        print(
            "[SEED] Superadmin security synchronized."
        )
        print(
        ...
        print(
            "[SEED] Superadmin security already configured."
        )
...
def run_seed() -> None:
    ...
    try:
        ...
        print(
            "[SEED] Seed failed. Transaction rolled back.",
            file=sys.stderr,
        )
