...
class AccountSecuritySettingsUpdate(BaseModel):
    ...
    verification_method: str = Field(
        default=AccountVerificationMethod.OTP_AND_EMAIL_LINK.value,
        max_length=40,
    )
    allow_login_before_verification: bool = False

    otp_length: int = Field(default=6, ge=4, le=10)
    otp_expiration_minutes: int = Field(default=15, ge=1, le=1440)
    otp_max_attempts: int = Field(default=5, ge=1, le=50)
    otp_resend_cooldown_seconds: int = Field(default=60, ge=0, le=86400)
    otp_max_resends_per_hour: int = Field(default=5, ge=1, le=100)
    email_link_expiration_minutes: int = Field(default=60, ge=1, le=10080)
    ...
    minimum_age: int = Field(default=18, ge=13, le=100)
    max_accounts_per_ip_per_day: int = Field(default=3, ge=1, le=1000)
    ...
    admin_mfa_totp_enabled: bool = True
    admin_mfa_recovery_codes_enabled: bool = True

    user_mfa_available: bool = False
    user_mfa_required: bool = False
    user_mfa_totp_enabled: bool = True
    user_mfa_recovery_codes_enabled: bool = True

    @field_validator("verification_method", mode="before")
    @classmethod
    def normalize_verification_method(cls, value: str) -> str:
...
class AccountSecuritySettingsResponse(BaseModel):
    ...
    admin_mfa_totp_enabled: bool
    admin_mfa_recovery_codes_enabled: bool

    user_mfa_available: bool
    user_mfa_required: bool
    user_mfa_totp_enabled: bool
    user_mfa_recovery_codes_enabled: bool
...
class AccountVerificationRequest(BaseModel):
    email: EmailStr
    purpose: str = Field(
        default=AccountVerificationPurpose.REGISTRATION.value,
        max_length=40,
    )
...
class AccountVerificationConfirmRequest(BaseModel):
    ...
    otp: str | None = Field(default=None, min_length=4, max_length=20)
...
class AccountVerificationChallengeListResponse(BaseModel):
    items: list[AccountVerificationChallengeResponse]
    total: int
    skip: int
    limit: int
