...
class BillingHistoryService:
    ...
    def _payment_commercial_summary(self, payment: BillingPayment) -> dict[str, Any]:
        ...
        plan = metadata.get("plan_snapshot") or metadata.get("subscription_plan_snapshot") or {}
        ...
        try:
            discount_percent = Decimal(str(percent_raw)).quantize(Decimal("0.000001")) if percent_raw not in (None, "") else None
        except Exception:
            discount_percent = None

        discount_code = metadata.get("coupon_code")
        discount_source = "coupon" if discount_code else (
            "plan" if metadata.get("plan_id") or plan else (
                "package" if metadata.get("token_package_id") else None
            )
        )
        discount_type = None
        if discount_amount > 0:
            discount_type = "percentage" if discount_percent is not None else "fixed_amount"
        ...
        return {
            ...
            "commercial_origin": str(terms.get("source") or metadata.get("purchase_kind") or payment.payment_type),
        }
    ...
    def list_payments(
        ...
        *,
        ...
        payments = billing_payment_repository.list_all_filtered(
            db,
            user_id=user_id,
            status=status_value,
        ...
        total = billing_payment_repository.count_filtered(
            db,
            user_id=user_id,
            status=status_value,
            payment_type=payment_type,
        ...
        return BillingInvoiceDocumentResponse(
            ...
            message=(
                ...
                if available
                    ...
                    "Invoice documents are not available yet. "
                    "The Stripe invoice may not be finalized."
                )
            ),
        )
    ...
    def reconcile_payment(
        ...
        *,
        ...
        if isinstance(latest_charge, str):
            ...
            payment.provider_charge_id = self._stripe_value(
                latest_charge,
                "id",
            )
        ...
        last_payment_error = self._stripe_value(
            payment_intent,
            "last_payment_error",
            {},
        ) or {}
    ...
    def refund_payment(
        ...
        *,
        ...
        if payment.payment_type == BillingPaymentType.TOKEN_PURCHASE.value:
            ...
            raise ConflictException(
                "Token purchases must be refunded through the "
                "token-purchase refund endpoint so purchased tokens "
                "can also be removed safely."
            )
        ...
        previous_refunded_amount = payment.refunded_amount

        stripe_refund = stripe_client_service.refund_payment_intent(
            db,
            payment_intent_id=(
                payment.provider_payment_intent_id
            ),
            amount_cents=self._money_to_cents(refund_amount),
            reason=data.reason,
            metadata={
