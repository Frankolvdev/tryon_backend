...
from app.services.billing_invoice_service import billing_invoice_service
...
class BillingService:
    ...
    def handle_verified_stripe_event(
        ...
        *,
        ...
        handlers = {
            ...
            "checkout.session.expired": self._checkout_failed,
            "checkout.session.async_payment_failed": self._checkout_failed,
            "payment_intent.succeeded": self._payment_intent_succeeded,
            "payment_intent.payment_failed": self._payment_intent_failed,
    ...
    def _checkout_completed(
        ...
        *,
        ...
        metadata = self._value(event_object, "metadata", {}) or {}
        checkout_type = self._value(metadata, "type")

        if checkout_type == "token_purchase":
            purchase = token_purchase_service.process_checkout_completed(
                db,
            ...
            return StripeWebhookResult(
                received=True,
                event_type=event_type,
                message="Token purchase was processed idempotently.",
                metadata={
                    "stripe_event_id": event_id,
    ...
    def _subscription_changed(
        ...
        *,
        ...
        subscription = subscription_service.sync_from_stripe_object(
            db,
    ...
    def _payment_intent_succeeded(
        ...
        *,
        ...
        payment_intent_id = self._value(event_object, "id")
        payment = billing_payment_repository.get_by_payment_intent_id(
            db,
            payment_intent_id,
        )
        if payment:
    ...
    def _payment_intent_failed(
        ...
        *,
        ...
        if payment:
            ...
            payment.failure_code = self._value(last_error, "code")
    ...
    def _refund_changed(
        ...
        *,
        ...
        if payment_intent_id:
            payment = (
                billing_payment_repository.get_by_payment_intent_id(
                    db,
                    payment_intent_id,
                )
            )
    ...
    def _charge_refunded(
        ...
        *,
        ...
        if payment_intent_id:
            ...
            purchase = (
                token_purchase_repository.get_by_payment_intent_id(
                    db,
                    payment_intent_id,
                )
            )
        ...
        if payment:
            payment.refunded_amount = (
                Decimal(amount_refunded) / Decimal("100")
