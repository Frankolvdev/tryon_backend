    ...
    def _copy_to_runpod(session: Any, models_root: Path, remote_path: str, overwrite: bool, notify: ProgressCallback) -> dict[str, Any]:
        ...
        for index, source in enumerate(files, start=1):
            ...
            def transfer_progress(bytes_amount: int) -> None:
                ...
                # the counter and UI throttling synchronized so callbacks never
                # serialize the actual multipart workers or flood job storage.
            ...
            notify(
                ...
                f"Iniciando {index} de {total_files}: {relative} ({human_bytes(file_size)}), transferencia paralela de hasta 8 partes…",
            )
            try:
                # Delegate multipart scheduling, retries, and parallelism to
                # boto3's S3Transfer manager. The former explicit upload_part
                # loop waited for every part before starting the next one, so
                # TransferConfig.max_concurrency never had any effect.
                s3.upload_file(
                    Filename=str(source),
                    Bucket=volume_id,
                    Key=key,
